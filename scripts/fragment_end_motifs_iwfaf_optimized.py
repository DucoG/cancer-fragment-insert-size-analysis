...
def read_pair_gen(bam_pointer: pysam.libcalignmentfile.AlignmentFile, region_str=None):
    read_dict = {}
    ...
    for read in bam_pointer.fetch(region=region_str):
        ...
        if both_mapped and correctly_oriented and primary_alignment:
            ...
            if query_name not in read_dict:
                read_dict[query_name] = [None, None]
...
def get_sequence(fasta_file: pysam.libcfaidx.FastaFile, reference, start, end, contig_length):
    if start < 0:
        padding = 'P' * abs(start)
...
def array_to_subsequent_unique(arr):
    yield arr[0]  # Start with the first element as True
    
    for i in range(1, len(arr)):
        if arr[i] != arr[i-1]:  # Check if the current element is different from the previous element
            yield arr[i]  # If different, yield it

def array_to_subsequent_unique_np(arr: list):
    arr = np.array(arr)
    mask = np.concatenate(([True], arr[1:] != arr[:-1]))
    return arr[mask]

def check_pattern(arr: np.array):
    if len(arr) < 3:
        return False
    elif len(arr) > 3:
        return True
    else:  # len(arr) == 3
        return not arr[0]  # returns True if the first element is False
    ...
    for contig in contigs:
        ...
        for read1, read2 in read_pair_gen(bam_pointer, region_str=contig):
            ...
            if contig_RPRs is None or fragment_length < 5:
                ...
                subsequent_unique_RPRs = array_to_subsequent_unique_np(contig_RPRs[fragment_start:fragment_end])
                # Determine if sub-array [False, True, False] is in array
                RPR_overlap = check_pattern(subsequent_unique_RPRs)
